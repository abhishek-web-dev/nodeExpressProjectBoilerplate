...
const { ERROR_CODES } = require('../../app/constants/error');
...
let validateRequest = (request, validator) => {
  ...
  if (validator.params) {
    ...
    if (result.error) throw new AppError(ERROR_CODES.REQUEST_PARAMS_INVALID);

    returnObj.params = params;
  }
  if (validator.query) {
    let result = validator.query.validate(query, { convert: true });
    if (result.error) throw new AppError(ERROR_CODES.REQUEST_QUERY_INVALID);
  ...
  if (validator.body) {
    ...
    if (result.error) throw new AppError(ERROR_CODES.REQUEST_BODY_INVALID);
